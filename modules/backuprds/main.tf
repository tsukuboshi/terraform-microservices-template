...
resource "aws_backup_plan" "tf_backup_plan" {
  name = "${var.system}-${var.project}-${var.environment}-${var.resourcetype}-plan-rds"

  rule {
    rule_name         = "${var.system}-${var.project}-${var.environment}-${var.resourcetype}-rule-rds"
...
resource "aws_backup_selection" "tf_backup_selection" {
  ...
  name         = "${var.system}-${var.project}-${var.environment}-${var.resourcetype}-rds"
