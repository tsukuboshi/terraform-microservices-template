...
resource "aws_wafv2_web_acl" "tf_wafv2_web_acl" {
  name  = "${var.system}-${var.project}-${var.environment}-${var.resourcetype}-webacl"
  scope = "REGIONAL"

  dynamic "default_action" {
    for_each = var.waf_default_action_block_mode ? [1] : []
    content {
      block {}
    }
  }
  ...
  rule {
    name     = "IPAdressRestriction"
    priority = 100

    action {
      dynamic "allow" {
        for_each = var.waf_ip_rule_allow_mode ? [1] : []
        content {}
      }
